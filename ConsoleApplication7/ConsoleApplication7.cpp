...
int Menu(int start);
void board();
void Move(int* move_rocket_l, int* move_rocket_r);
...
void board() {
	...
	while (L_P_point != 10 && R_P_point != 10) {
		system("cls");
		printf("LR%d RR%d %d %d\n", rocketl,rocketr, Ball_X, Ball_Y);
		...
		Move(move_rocket_l, move_rocket_r);
...
void Move(int* move_rocket_l, int* move_rocket_r) {
	char c;
	scanf("%c", &c);
	if (c == 'a' && *move_rocket_l > -10) {
		*move_rocket_l -= 1;
	} else if (c == 'z' && *move_rocket_l < 10) {
		*move_rocket_l += 1;
	} else if (c == 'k' && *move_rocket_r > -10) {
		*move_rocket_r -= 1;
	} else if (c == 'm' && *move_rocket_r < 10) {
		*move_rocket_r += 1;
	} else if (c == ' ') {
		*move_rocket_l += 0;
		*move_rocket_r += 0;
